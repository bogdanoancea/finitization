...
test_that("Output and return value for an invalid 'val' (nonexistent index) are handled correctly (non-LaTeX)", {
    capture.output(out <- suppressMessages(printFinitizedPoissonDensity(2, val = 3)))

    # Expect no printed output.
    expect_equal(length(out), 0)
    # And the function should return NULL.
    expect_null(suppressMessages(printFinitizedPoissonDensity(2, val = 3)))
})

test_that("Output and return value for an invalid 'val' (nonexistent index) are handled correctly (LaTeX)", {
    capture.output(out <- suppressMessages(printFinitizedPoissonDensity(2, val = 3, latex = TRUE)))

    expect_equal(length(out), 0)
    expect_null(suppressMessages(printFinitizedPoissonDensity(2, val = 3, latex = TRUE)))
})
