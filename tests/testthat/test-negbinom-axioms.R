# tests/testthat/test-negbinom-axioms.R

test_that("Finitized Negative Binomial satisfies probability axioms (pmf sums to 1, bounds, CDF hits 1)", {

    # Stay safely inside MFPS by choosing small q (as in your paper examples)
    # and moderate n. Adjust/extend as you like.
    cases <- list(
        list(n = 2, k = 2, q = 0.10),
        list(n = 4, k = 2, q = 0.15),
        list(n = 8, k = 2, q = 0.05)
    )

    tol_sum <- 1e-12
    tol_cdf <- 1e-12

    for (cs in cases) {
        n <- cs$n
        k <- cs$k
        q <- cs$q
        x <- 0:n

        # PMF over full support
        pmf_df <- dnegbinom(n = n, q = q, k = k, val = x)
        pmf <- pmf_df$prob

        # 1) Sum to unity
        expect_equal(sum(pmf), 1.0, tolerance = tol_sum)

        # 2) Valid probability values
        expect_true(all(is.finite(pmf)))
        expect_true(all(pmf >= -tol_sum))      # allow tiny numerical negatives
        expect_true(all(pmf <= 1 + tol_sum))

        # 3) CDF reaches 1 at upper bound
        cdf_df <- pnegbinom(n = n, q = q, k = k, val = n)
        expect_equal(cdf_df$cdf[1], 1.0, tolerance = tol_cdf)

        # Extra: monotone CDF within [0,1]
        cdf_all <- pnegbinom(n = n, q = q, k = k, val = x)$cdf
        expect_true(all(diff(cdf_all) >= -tol_sum))
        expect_true(all(cdf_all >= -tol_sum))
        expect_true(all(cdf_all <= 1 + tol_sum))
    }
})
