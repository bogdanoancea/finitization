...
defects_trunc <- rtrunc(n = n_batches, spec = "binom", 
...
sample_trunc <- rtrunc(n = n_large, spec = "binom", 
                       a = -1, b = n, size = N, prob = p)
...
ggplot(pmf_data, aes(x = k, y = Probability, fill = Method)) +
  geom_col(position = position_dodge(width = 0.8), alpha = 0.7) +
  geom_vline(xintercept = d + 0.5, linetype = "dashed", color = "red") +
  scale_fill_manual(values = c("Theoretical" = "#E63946",
                               "Finitized" = "#2E86AB", 
                               "Truncated" = "#A23B72",
                               "Capped" = "#F18F01")) +
  labs(
  ...
  annotate("text", x = d + 0.5, y = max(pmf_data$Probability) * 0.9,
...
ggplot(error_data, aes(x = Method, y = Error, fill = Method)) +
  geom_col(alpha = 0.7) +
  geom_hline(yintercept = 0, linetype = "solid") +
  geom_text(aes(label = sprintf("%.2f%%", Error)), 
            vjust = ifelse(error_data$Error >= 0, -0.5, 1.5),
            size = 3.5) +
  facet_wrap(~ Moment, scales = "free_y") +
  scale_fill_manual(values = c("Finitized" = "#2E86AB", 
                               "Truncated" = "#A23B72",
                               "Capped" = "#F18F01")) +
  labs(
