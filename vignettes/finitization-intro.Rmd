...
and logarithmic distributions [@golnabi2009finitizing].
...
*Sketch.* Write $\tau(\theta)=\sum_{j=0}^{\infty} b_j \theta^j$. Since
$\eta(\theta)\tau(\theta)\equiv 1$, equating coefficients yields $c_0=1$
...
the next sections, guarantees that the first $n$ moments of the
...
differentiable on $(-c, c)$ for some $c > 0$). For constants $a$ and $b$
...
Now set $a = -\lambda$ and $b = \lambda$, with $\lambda > 0$. If
$\delta^{(k)}(-\lambda) \geq 0$ for all $k$, t
...
In summary, CRAN installation should work out of the box on all
