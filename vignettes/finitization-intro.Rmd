...
versions of several well-known members of the *power series family* distributions
...
2.  **Open the “MSYS2 UCRT 64‑bit” shell** Supposing that RTools45 is
...
The `r*` functions in **finitization** implement a hybrid approach: a simple CDF 
ladder approach for small finitization orders (currently $n \le7$) on macOs platform
and an **aliasing**-based generator for the rest of the cases, to produce random variates on the finite support
...
- If \(\texttt{macOS} \,\land\, K \le K_{max}\), use a small-support CDF ladder
  (fully unrolled).
