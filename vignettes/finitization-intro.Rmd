...
transforms a discrete distribution - often one with infinite
...
mass functions and random variates, and practical guidance for choosing
the finitization order *n*. This structure allows the reader to both
...
distribution - most often a member of the power series family such as
...
Another area where finitization can be used is in **random variate
generation**. Simulation methods such as *aliasing* (efficient for
...
variate generation whil preserving a close fit to the parent
distribution.
...
and logarithmic distributions [@golnabi2009finitizing]. For the Poisson distribution th PSD base function can be expressed as $\eta(\theta)=\exp(\theta)$.


If the parent PSD has infinite support, finitization creates a new
distribution supported only on\
$\{0,1,\dots,n\}$. The **partial sum finitization (PSF)** of order $n$
is defined as
...
For a PSD random variable $X$ with pmf in (1)–(2), the **moment
...
differentiable on $(-c, c)$ for some $c > 0$). We call it the base function. For constants $a$ and $b$
such that $a+b \in (-c,c)$, we have the Taylor expansion
...
which gives a distribution with support over the nonnegative integers
and a pdf / pmf
...
-   The **lower bound** $\theta_{\min}$ is determined by examining when
    finitized probabilities near the lower edge of the support first
...
-   **Feasibility guarantee:** MFPS ensures that the finitized
    distribution is a proper pdf (all probabilities are nonnegative,
    summing to 1).
...
2.  **Open the “MSYS2 UCRT 64‑bit” shell** Supposing that RTools44 is
    installed in "C:/rtools44" then double click on
    "C:/rtools44/ucrt64.exe".
...
platforms. Manual configuration is required only when compiling from
source on Linux or when building custom versions of CLN and GiNaC on
Windows.
...
respectively. This function is particularly useful for simulation
studies, where random draws may be generated via inversion.
...
while adjusting the probability mass function through the
moment-preserving finitization series (MFPS) [@levy2012moment]. This
...
parameterized by: - **finitization order** $n$, which sets the support
$\{0,\dots,n\}$;\
- **success probability** $p$;\
- **number of trials** $N$.

The subsections below present the available functions and illustrate
their use.
...
The probability mass function (PMF) of the finitized binomial
...
The cumulative distribution function (CDF) is available via `pbinom()`.
...
distribution by inverting the CDF. Given probability levels (`u`), it
...
parameter space (MFPS) for the finitized binomial distribution. The MFPS
...
Returns the finitized Poisson probability mass function (PMF). With
...
$$ This corresponds to a log-series shifted to include 0 in its support,
with pmf $$
...
```
## ARM64–Specific Optimizations for Small Finitization Orders
The package places strong emphasis on achieving
architecture-aware performance for random variate generation.
The default generator uses an optimized Vose–Walker alias table with a
16-way unrolled inner loop.  
This method performs extremely well on x86\_64 (Linux/Windows), where indirect
branches are handled efficiently.

On ARM64 processors, however—particularly Apple’s M1/M2/M3/M4 designs—the
microarchitectural balance is different.  
Indirect and hard-to-predict branches carry a noticeably higher penalty
than on x86\_64, and this affects the alias method because each draw requires
an unpredictable conditional:
\[
  X = (f < \mathrm{cutoff}[j])\;\; ?\;\; j : \mathrm{alias}[j],
  \qquad j = \lfloor U K \rfloor .
\]
When the support size is very small, this branch becomes the dominant source
of latency.

To address this, \texttt{finitization} performs an automatic switch to a
small-support CDF ladder whenever the support size
\[
K = n + 1 \le 4
\]
and the code is running on an ARM64 architecture (e.g. Apple Silicon).

During \texttt{setProbs()}, the normalized PMF is cached; for tiny \(K\),
...
This architecture-aware dispatch yields substantial improvements on ARM64
for finitization orders with \(K \le 4\). In our benchmarks:
...
On x86\_64, the alias generator remains optimal for practically all
finitization orders and is therefore used everywhere. On ARM64, the
small–\(K\) CDF ladder is selected automatically for tiny supports, while the
alias method is retained for larger supports.

### Dispatch Logic

The selection between the alias generator and the CDF ladder is internal
and transparent to the user; it can be summarized as in Figure \ref{fig:arm-dispatch}.
...
- Detect whether the code is running on an ARM64 architecture.
- If \(\texttt{ARM64} \,\land\, K \le 4\), use a tiny-support CDF ladder
  (fully unrolled).
