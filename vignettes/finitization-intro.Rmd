...
Ah, I see the confusion 🙂 — you want everything (both the explanatory text and the code) written directly in R Markdown style, not just code fences sprinkled into plain text. Here is the whole section again, properly structured as Rmd (you can drop this straight into your vignette .Rmd file):
...
Or directly from R with devtools:
...
This is now fully in **Rmd** style: narrative text is plain Markdown, and commands are in fenced chunks with language tags (`{r}`, `{bash}`).  

👉 Do you also want me to prepend a **vignette header** (YAML with `title`, `author`, `output: rmarkdown::html_vignette`) so it compiles as a self-contained vignette section?
