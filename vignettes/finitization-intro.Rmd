...
This section shows how to work with the finitized Negative Binomial distribution provided by
`finitization`. The finitization order is `n` (support `0:n`), the success probability per trial is `q`,
and `k > 0` is the number of failures until stopping.

The classical negative binomial distribution models the number of failures before
a fixed number of successes, with support on all nonnegative integers. In practice, many systems
impose a natural upper bound (e.g., a finite number of trials). The finitized version restricts
the support to `{0,1, ... ,n}` while preserving the first `n` moments of the parent distribution.

The finitized negative binomial is particularly useful in scenarios such as:

- modeling the number of infection events in epidemiology when only a finite number of trials
  can occur
- bounded population studies where counts cannot exceed a fixed capacity
- or simulation studies where efficient random variate generation is required.

In what follows we present a series of examples that demonstrate how to use the finitized
negative binomial distribution functions implemented in the `finitization` package. We cover
the probability mass function (PMF), cumulative distribution function (CDF), quantiles,
random variate generation, as well as utilities for extracting finitization coefficients
and displaying the density in closed form.
...
q <- 0.10   # success probability per trial. Be careful when choosing q to be inside MFPS
k <- 2      # number of failures until the experiment stops
x <- 0:n
```
...
The following code illustrates these diagnostic procedures in practice.
