#include "Finitization.h"

using namespace std;

#include <cfloat>    // DBL_EPSILON
#include <cmath>     // std::fabs
#include <limits>
#include <algorithm>
#include <vector>
#include <ginac/ginac.h>

#ifndef RESTRICT
#if defined(__GNUC__) || defined(__clang__)
#define RESTRICT __restrict__
#else
#define RESTRICT
#endif
#endif

// Compile-time cap for when we even consider the CDF-ladder path.
// This can be ANY positive integer; correctness does not depend on its value.
#ifndef K_LADDER_MAX
#define K_LADDER_MAX 8
#endif

// If you want the ladder only on macOS builds, keep this default.
// You can override at compile time: -DFINITIZATION_ENABLE_CDF_LADDER=0/1
#ifndef FINITIZATION_ENABLE_CDF_LADDER
#if defined(__APPLE__)
#define FINITIZATION_ENABLE_CDF_LADDER 1
#else
#define FINITIZATION_ENABLE_CDF_LADDER 0
#endif
#endif

// --------------------------------------------------------------
// Helpers: build an ordered CDF ladder and sample from it.
// Correct for ANY K and ANY K_LADDER_MAX.
// --------------------------------------------------------------

// Build CDF in the order given by idx[0..K-1] (idx contains original outcomes).
// pmf must be normalized (sum ~ 1). We enforce cdf[K-1] = 1.0 exactly.
static inline void build_ordered_cdf(
        const double* RESTRICT pmf,  // normalized pmf in original order [0..K-1]
        const int*    RESTRICT idx,  // permutation of {0..K-1}, length K
        int K,
        double* RESTRICT cdf          // output, length K
) {
    double acc = 0.0;
    for (int r = 0; r < K; ++r) {
        acc += pmf[idx[r]];
        cdf[r] = acc;
    }
    if (K > 0) cdf[K - 1] = 1.0;
}

// Always-correct sampler: returns original outcome (idx[pos]).
static inline int sample_ordered_cdf_loop(
        const double* RESTRICT cdf,  // increasing, cdf[K-1]==1
        const int*    RESTRICT idx,
        int K,
        double u
) {
    // Find first r such that u <= cdf[r]
    for (int r = 0; r < K; ++r) {
        if (u <= cdf[r]) return idx[r];
    }
    // Numerical safety (should be unreachable if cdf[K-1]==1)
    return idx[K - 1];
}

// --------------------------------------------------------------
// Finitization class
// --------------------------------------------------------------

Finitization::Finitization(int n)
    : m_finitizationOrder(n),
      m_dprobs{nullptr},
      m_finish{false}
      {
          const int K = m_finitizationOrder + 1;

          // Memory allocation for alias method
          m_alias  = new int[K];
          m_prob   = new double[K];
          m_values = new int[K];

          m_ntsfFirstTime = true;
          for (int i = 0; i <= n; i++)
              m_values[i] = i;

          // optional small-K PMF cache (allocated lazily in setProbs)
          m_pmf_small = nullptr;
          m_smallK    = false;
      }

Finitization::~Finitization() {
    delete[] m_alias;
    delete[] m_prob;
    delete[] m_values;
    delete[] m_dprobs;
    if (m_pmf_small) {
        delete[] m_pmf_small;
        m_pmf_small = nullptr;
    }
}

void Finitization::setProbs(double* p) {
    const int K = m_finitizationOrder + 1;
    if (K <= 0) stop("Internal: K <= 0 in setProbs().");

    // 1) Normalize p -> pmf (we cache normalized pmf for ladder)
    double sum = 0.0;
    for (int i = 0; i < K; ++i) {
        if (p[i] < 0.0) stop("Negative probability at index %d.", i);
        sum += p[i];
    }
    if (sum <= 0.0) stop("Sum of probabilities is zero in setProbs().");
    const double inv_sum = 1.0 / sum;

    // --- small-K PMF cache for CDF ladder ---
    if (K <= K_LADDER_MAX) {
        if (!m_pmf_small) {
            m_pmf_small = new double[K];
        }
        for (int i = 0; i < K; ++i) {
            m_pmf_small[i] = p[i] * inv_sum;  // normalized pmf
        }
        m_smallK = true;
    } else {
        if (m_pmf_small) {
            delete[] m_pmf_small;
            m_pmf_small = nullptr;
        }
        m_smallK = false;
    }

    // 2) Scale by K (Vose/Walker) and partition into small/large
    double* P = new double[K];
    for (int i = 0; i < K; ++i)
        P[i] = (p[i] * inv_sum) * static_cast<double>(K);

    int* S = new int[K];
    int* L = new int[K];
    int nS = 0, nL = 0;

    for (int i = K - 1; i >= 0; --i) {
        (P[i] < 1.0) ? (S[nS++] = i) : (L[nL++] = i);
    }

    // 3) Build alias table
    while (nS && nL) {
        const int a = S[--nS];
        const int g = L[--nL];

        m_prob[a]  = P[a];
        m_alias[a] = g;

        P[g] = (P[g] + P[a]) - 1.0;
        (P[g] < 1.0) ? (S[nS++] = g) : (L[nL++] = g);
    }

    // 4) Leftovers
    while (nL) {
        const int i = L[--nL];
        m_prob[i]  = 1.0;
        m_alias[i] = i;
    }
    while (nS) {
        const int i = S[--nS];
        m_prob[i]  = 1.0;
        m_alias[i] = i;
    }

    delete[] P;
    delete[] S;
    delete[] L;
}

IntegerVector Finitization::rvalues(int no) {
    if (no < 0) {
        stop("'no' must be nonnegative.");
    }

    const int K  = m_finitizationOrder + 1;
    const double* RESTRICT cutoff = m_prob;
    const int*    RESTRICT alias  = m_alias;

    IntegerVector out(no);
    int* RESTRICT p = out.begin();

    GetRNGstate();

#if FINITIZATION_ENABLE_CDF_LADDER
    // ===== Small-K CDF ladder path (correct for ANY K_LADDER_MAX) =====
    if (K <= K_LADDER_MAX && m_pmf_small != nullptr) {
        // Build probability-ordered index list idx[0..K-1] (descending pmf)
        // NOTE: idx contains original outcome values (0..K-1)
        std::vector<int> idx(K);
        for (int i = 0; i < K; ++i) idx[i] = i;

        // Sort by descending probability mass (stable is fine for reproducibility)
        std::stable_sort(idx.begin(), idx.end(),
                         [&](int a, int b) { return m_pmf_small[a] > m_pmf_small[b]; });

        // Build ordered CDF ladder
        std::vector<double> cdf(K);
        build_ordered_cdf(m_pmf_small, idx.data(), K, cdf.data());

        // Unroll the DRAW LOOP (not the ladder depth). This is safe for any K.
        // Use a fixed unroll factor for throughput.
        constexpr int B = 16;
        int i = 0;

        for (; i + B - 1 < no; i += B) {
            // 16 independent draws
            p[i + 0]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 1]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 2]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 3]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 4]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 5]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 6]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 7]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 8]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 9]  = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 10] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 11] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 12] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 13] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 14] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
            p[i + 15] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
        }

        for (; i < no; ++i) {
            p[i] = sample_ordered_cdf_loop(cdf.data(), idx.data(), K, unif_rand());
        }

        PutRNGstate();
        return out;
    }
#endif

    // ===== General alias path for all other cases / platforms =====
    const double Kd = static_cast<double>(K);
    constexpr int UN = 16;
    const int nU = (no / UN) * UN;

    int t = 0;
    for (; t < nU; t += UN, p += UN) {
        double uK0 = unif_rand() * Kd;  const uint32_t j0  = (uint32_t)uK0;  const double f0  = uK0  - (double)j0;
        p[0]  = (f0  < cutoff[j0])  ? (int)j0  : alias[j0];

        double uK1 = unif_rand() * Kd;  const uint32_t j1  = (uint32_t)uK1;  const double f1  = uK1  - (double)j1;
        p[1]  = (f1  < cutoff[j1])  ? (int)j1  : alias[j1];

        double uK2 = unif_rand() * Kd;  const uint32_t j2  = (uint32_t)uK2;  const double f2  = uK2  - (double)j2;
        p[2]  = (f2  < cutoff[j2])  ? (int)j2  : alias[j2];

        double uK3 = unif_rand() * Kd;  const uint32_t j3  = (uint32_t)uK3;  const double f3  = uK3  - (double)j3;
        p[3]  = (f3  < cutoff[j3])  ? (int)j3  : alias[j3];

        double uK4 = unif_rand() * Kd;  const uint32_t j4  = (uint32_t)uK4;  const double f4  = uK4  - (double)j4;
        p[4]  = (f4  < cutoff[j4])  ? (int)j4  : alias[j4];

        double uK5 = unif_rand() * Kd;  const uint32_t j5  = (uint32_t)uK5;  const double f5  = uK5  - (double)j5;
        p[5]  = (f5  < cutoff[j5])  ? (int)j5  : alias[j5];

        double uK6 = unif_rand() * Kd;  const uint32_t j6  = (uint32_t)uK6;  const double f6  = uK6  - (double)j6;
        p[6]  = (f6  < cutoff[j6])  ? (int)j6  : alias[j6];

        double uK7 = unif_rand() * Kd;  const uint32_t j7  = (uint32_t)uK7;  const double f7  = uK7  - (double)j7;
        p[7]  = (f7  < cutoff[j7])  ? (int)j7  : alias[j7];

        double uK8 = unif_rand() * Kd;  const uint32_t j8  = (uint32_t)uK8;  const double f8  = uK8  - (double)j8;
        p[8]  = (f8  < cutoff[j8])  ? (int)j8  : alias[j8];

        double uK9 = unif_rand() * Kd;  const uint32_t j9  = (uint32_t)uK9;  const double f9  = uK9  - (double)j9;
        p[9]  = (f9  < cutoff[j9])  ? (int)j9  : alias[j9];

        double uK10 = unif_rand() * Kd; const uint32_t j10 = (uint32_t)uK10; const double f10 = uK10 - (double)j10;
        p[10] = (f10 < cutoff[j10]) ? (int)j10 : alias[j10];

        double uK11 = unif_rand() * Kd; const uint32_t j11 = (uint32_t)uK11; const double f11 = uK11 - (double)j11;
        p[11] = (f11 < cutoff[j11]) ? (int)j11 : alias[j11];

        double uK12 = unif_rand() * Kd; const uint32_t j12 = (uint32_t)uK12; const double f12 = uK12 - (double)j12;
        p[12] = (f12 < cutoff[j12]) ? (int)j12 : alias[j12];

        double uK13 = unif_rand() * Kd; const uint32_t j13 = (uint32_t)uK13; const double f13 = uK13 - (double)j13;
        p[13] = (f13 < cutoff[j13]) ? (int)j13 : alias[j13];

        double uK14 = unif_rand() * Kd; const uint32_t j14 = (uint32_t)uK14; const double f14 = uK14 - (double)j14;
        p[14] = (f14 < cutoff[j14]) ? (int)j14 : alias[j14];

        double uK15 = unif_rand() * Kd; const uint32_t j15 = (uint32_t)uK15; const double f15 = uK15 - (double)j15;
        p[15] = (f15 < cutoff[j15]) ? (int)j15 : alias[j15];
    }

    for (int r = nU; r < no; ++r, ++p) {
        const double uK = unif_rand() * Kd;
        const uint32_t j = (uint32_t)uK;
        const double   f = uK - (double)j;
        *p = (f < cutoff[j]) ? (int)j : alias[j];
    }

    PutRNGstate();
    return out;
}

GiNaC::ex Finitization::ntsf(GiNaC::ex pnb) {
    if (m_ntsfFirstTime) {
        m_ntsfSymb = series_to_poly(pnb.series(m_x == 0, m_finitizationOrder + 1));
        m_ntsfFirstTime = false;
    }
    return m_ntsfSymb;
}

GiNaC::ex Finitization::pdf(GiNaC::ex ntsf, int x_val) {
    GiNaC::ex optheta = -m_paramSymb;
    GiNaC::ex pdf;
    if (x_val > 1 && m_cache.find(x_val - 1) != m_cache.end()) {
        pdf = m_cache[x_val - 1].diff(m_x, 1);
        m_cache.insert({x_val, pdf});
    } else {
        pdf = ntsf.diff(m_x, x_val);
        m_cache.insert({x_val, pdf});
    }

    pdf = pdf.subs(m_x == optheta) * pow(m_paramSymb, x_val) / factorial(x_val);
    return pdf;
}

GiNaC::ex Finitization::fin_pdfSymb(int x_val) {
    GiNaC::ex pdf_ = pdf(ntsf(ntsd_base(m_x, m_paramSymb)), x_val);
    return pdf_;
}

// Map the expression tree, turning numerics into doubles, and zeroing those
// strictly below machine epsilon (absolute).
GiNaC::ex map_double_and_zero_eps(const GiNaC::ex& e) {
    using namespace GiNaC;
    if (is_a<numeric>(e)) {
        const double d = ex_to<numeric>(e).to_double();
        if (std::fabs(d) < DBL_EPSILON)
            return numeric(0.0);
        return numeric(d);
    }
    return e.map(map_double_and_zero_eps);
}

std::string Finitization::pdfToString(int val, bool tolatex) {
    std::stringstream result;
    GiNaC::ex pdf_ = fin_pdfSymb(val);

    pdf_ = map_double_and_zero_eps(pdf_);

    if (tolatex)
        result << latex;
    result << pdf_;
    return result.str();
}

double Finitization::fin_pdf(int val) {
    if (m_finish && val <= m_finitizationOrder)
        return m_dprobs[val];
    else {
        GiNaC::ex pdf_ = fin_pdfSymb(val);
        double tmp = GiNaC::ex_to<GiNaC::numeric>(
            evalf(pdf_.subs(m_paramSymb == m_theta))
        ).to_double();

        const double eps   = std::numeric_limits<double>::epsilon();
        const double atmp  = std::abs(tmp);
        const double scale = std::max(1.0, atmp);
        const double tol   = 64.0 * eps * scale + 1e-300; // denormal floor

        double x = (atmp <= tol) ? 0.0 : tmp;  // zero tiny magnitudes
        x = std::max(x, 0.0);
        return x;
    }
}
